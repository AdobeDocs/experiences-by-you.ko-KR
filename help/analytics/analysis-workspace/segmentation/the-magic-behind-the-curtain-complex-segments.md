...
_복잡한 데이터 세분화의 복잡성을 파악하고 제외, 컨테이너 및 속성 모델을 탐색합니다. 마술사의 솜씨처럼 이러한 기술을 숙달하면 분석가가 데이터 마술을 수행할 수 있어 정밀하고 정교하게 통찰력을 전환할 수 있습니다._

커튼이 열려 있고 무대가 설정되어 있습니다... 이것은 라스베가스의 마법 행위가 아닐 수 있지만, 우리는 우리의 세그먼트를 만들 때 몇 가지 꽤 놀라운 트릭을 수행 할 수 있습니다.

![마술사_핸즈](assets/magician-hands.jpeg)
...
기본적으로 모든 컨테이너는 다음으로 시작합니다. **include** 을 입력합니다. 기본적으로 기준에 일치하는 데이터를 반환합니다. 그러나 세그먼트 또는 세그먼트 내의 컨테이너를 다음과 같이 변경할 수도 있습니다 **제외** 을 입력하여 특정 기준을 거부할 수 있습니다.

마술사는 갑판에서 카드를 찾을 수 있지만, 그 마술사가 갑판의 나머지를 존재하지 않게 만들 수 있다는 것은 놀라운 일입니다. 마찬가지로, 세그먼트 제외에서 원하지 않는 데이터가 데이터 세트에서 사라지기를 원합니다.

![공백 카드](assets/blankcards.png)

여러분은 아마 거기에 앉아 &quot;좋아, 하지만 나는 이미 &#39;같지 않음&#39;과 &#39;포함하지 않음&#39;옵션을 가지고 있는데, 그래서 나를 가려야 하지 않을까?&quot;라고 생각할지도 모른다. 불행히도, 그것에 대한 대답은... 그리고 그것은 단지 하나의 요소를 통해 논리의 그룹을 제외할 수 있음에 대한 것이 아닙니다. 단일 구성 요소를 처리하는 경우에도 을 사용해야 하는 경우가 많습니다 *제외* 목표를 달성하기 위해.

- **다음을 포함하지 않음 / 다음과 같지 않음** - 특정 문자열을 포함하지 않는 항목에 일치하는 것처럼 들립니다.
- **제외: 값에 포함 / 같음** - 이는 다음과 같습니다. *제외* 문자열과 일치하는 항목

언뜻 보기에는 둘 다 같은 소리로 들립니다... **히트** 세그먼트/컨테이너를 레벨링하면 동일한 작업을 수행하므로 올바른 것입니다. 단, 을 사용할 때는 **방문** 또는 **방문자** 범위는 매우 다른 결과를 얻을 수 있습니다.

**그림 1: 다음을 포함하지 않음 / 같지 않음 - 히트 범위**
...
- Value에 &quot;Example&quot;(아니요)이 포함되어 있으면 false를 반환하고 해당 히트를 제외하지 않습니다. 마찬가지로, &quot;Example&quot;에도 &quot;Example&quot;(예)이 포함되어 있으면 true를 반환하고 해당 히트를 제외합니다. 기본적으로 다음을 수행하는 데이터를 반환합니다. **아님** true 결과를 얻거나 false인 데이터를 기준에 반환합니다.
- 다음에서 볼 수 있습니다. **히트** level을 지정하면 두 논리 세트 모두 동일한 데이터 세트를 반환합니다.

**그림 2: 포함하지 않음 / 같지 않음 - 방문 범위**

![그림2-DenceVsExclude-Visit](assets/figure2-dnce-vs-exclude-visit.png)

*위와 같이 각 히트는&#x200B;**방문**동일한 true/false로 평가됩니다. 그러나 반환되는 데이터 세트는 전체 방문의 데이터 세트입니다.*

- 각 히트에서 &quot;Value&quot;는 &quot;Example&quot;(예)을 포함하지 않으므로 true를 반환합니다. 마찬가지로 &quot;Example&quot;도 &quot;Example&quot;(아니요, 포함합니다.)을 포함하지 않으므로 false를 반환합니다.
   - If **임의** 방문의 히트가 반환됩니다 **true**, 그런 다음 **전체 방문** 이 반환됩니다.*
   - 방문이 전적으로 &quot;예&quot;를 포함한 히트로 이루어졌다면 어떤 히트도 true를 반환하지 않으므로 해당 방문은 다음과 같이 변경됩니다 **반환되지 않음** 을 참조하십시오.
- 다시 말하지만, 각 히트에서 &quot;Example&quot;은 &quot;Example&quot;(예)을 포함하므로 true를 반환합니다
   - If **모든 히트** 반환 **true**, 전체 방문 은 다음과 같습니다. **제외됨**
   - If **모든 히트 수** 방문 반환 시 **false**, 그러면 데이터 세트에서 해당 방문이 반환됩니다
- 이제 이 논리가 어디에서 갈라지기 시작하는지 알 수 있다. 위의 예에는 세 개의 개별 방문이 있습니다.
   - &quot;포함하지 않음 / 같음&quot; 사용 시 **셋 중 두 개** 방문이 반환됩니다.
   - &quot;포함/다음과 같음 제외&quot;를 사용할 때 **하나만** 이러한 방문 중 하나가 반환됩니다.

**그림 3: 포함하지 않음 / 같지 않음 - 방문 범위**

![그림3-DnceVsExclude-Visitor](assets/figure3-dnce-vs-exclude-visitor.png)

*위와 같이, 히트는&#x200B;**방문자**은 동일한 true/false 논리로 평가됩니다. 그러나 이제 이 방문자가 모든 방문 (선택한 날짜 범위 내)에 대해 수행한 모든 히트를 살펴봅니다.*

- 각 히트에서 &quot;Value&quot;는 &quot;Example&quot;(예)을 포함하지 않으므로 true를 반환합니다. 마찬가지로 &quot;Example&quot;도 &quot;Example&quot;(아니요, 포함합니다.)을 포함하지 않으므로 false를 반환합니다.
   - If **임의** 방문자가 수행한 히트는 **true**, 그런 다음 **전체 방문** 가 반환됩니다.
   - 방문자가 &quot;Example&quot;을 포함하는 히트를 수행하지 않았다면 어떤 히트도 true를 반환하지 않으므로 해당 방문자는 **반환되지 않음** 을 참조하십시오.
- 다시 말하지만, 각 히트에서 &quot;Example&quot;은 &quot;Example&quot;(예)을 포함하므로 true를 반환합니다.
   - If **모든 히트** 반환 **true**, 전체 방문자(및 결과적으로 모든 방문)는 **제외됨.**
   - If **모든 히트 수** 방문 반환 시 **false**, 그러면 해당 방문자가 데이터 세트에서 반환되어 &quot;X&quot;를 수행하지 않은 방문자가 성공적으로 반환됩니다.
- 이는 훨씬 더 많은 고려 사항이 있는 방문 논리의 확장입니다. 위의 예에는 두 명의 고유 방문자가 있으며 각각 3개의 방문이 있습니다.
   - &quot;포함하지 않음 / 같음&quot; 사용 시 **모두** 모든 방문자와 마찬가지로 방문자가 반환됩니다 **3** 방문의 수 (보고서에서 2명의 방문자와 총 6개의 방문을 차지함)
   - &quot;포함/다음과 같음 제외&quot;를 사용할 때 **하나만** 이러한 방문자 중 가 반환되고, 해당 방문자와 연결된 세 개의 방문만 포함됩니다 (보고서에서 1명의 방문자와 3개의 총 방문을 차지함)
...
이 예에서는 사이트로 이동하고 이동한 사용자를 타겟팅하려고 합니다 *아님* 방문 중에 구매(기본적으로 트랜잭션을 수행한 방문은 제외하려고 하므로 트랜잭션을 완료하지 않은 방문은 그대로 둡니다.)
...
먼저, 당신은 그럼에도 불구하고 그것을 볼 수 있습니다. *방문* 세그먼트의 레벨 범위에서는 세그먼트를 다른 지표(예: 페이지 보기 수 또는 고유 방문자 수)와 쌍으로 연결할 수 있습니다. 첫 번째 열 세트는 세그먼트화되지 않으므로 한 세그먼트(존재하지 않음)가 거의 100%의 데이터를 반환하고 있음을 한 눈에 볼 수 있으며, 제외 세그먼트만 필요한 작업을 수행하고 있습니다.
...
이제 방문자의 범위 지정 세그먼트를 위의 방문 범위 지정 세그먼트와 비교하면 다음과 같은 이유로 더 많은 데이터와 더 많은 방문이 제외됩니다 *구매한 방문자* 또한 구매가 수행되지 않은 방문이 있으므로 해당 방문도 방문자의 라이프사이클에 포함되므로 제외됩니다.
...
>[!IMPORTANT]
>
>반면에 방문과 방문자의 차이점은 다음과 같을 수 있습니다. *미묘하* (특히 이 샘플 데이터에서) 이들은 고려해야 하는 고유한 논리입니다. 데이터는 사이트 및 사용자 동작에 따라 상당히 다를 수 있습니다.


정확히 어떤 데이터인지 또는 무엇을 알아야 합니다. *story*, 보고서와 구분하려고 합니다. 테이블 및 시각화가 대상자에게 명확하게 전달되는지 확인 ***내용*** 가 표시되고 있으며 적절한 세그먼트 모델을 사용하는 것은 적절한 분석을 수행하는 데 중요합니다. 정보에 입각한 결정은 모두가 그들이 보고 있는 것을 이해해야만 제대로 이루어질 수 있다.
...
먼저 다음에 대한 간단한 분류를 수행합니다. *컨테이너* 범위. 좋아요 *세그먼트 s*&#x200B;코프, 넌 기본을 갖고있어 **히트**, **방문** 및 **방문자** 범위 옵션, 하지만 때로는 **논리 그룹** 방문자 대신(순차적 세그먼트 내에서만 발생하며 다음 문서에서 그러한 내용을 다룹니다.)

세그먼트 내에(또는 다른 컨테이너 내에) 컨테이너를 추가하는 작업은 **옵션*** 메뉴(여러 항목을 중첩한 경우 올바른 블록에 추가하는 것을 주의하십시오. 잘못된 위치에 추가한 경우 인터페이스 내에서 컨테이너를 끌어서 놓을 수 있습니다.)

**그림 1: 컨테이너 추가**

![그림 1-AddingContainer](assets/figure1-adding-container.png)

컨테이너의 범위는 위에서 언급했듯이 부모와 독립적입니다 *금지* 일치해야 하며, 반환하려는 내용에 따라 적어도 머릿속에서 편안하게 시각화할 때까지 필요한 것을 완전히 시각화하기 위해 계획을 짜야 할 수도 있습니다.

**그림 2: 세그먼트 범위와 컨테이너 범위**

![그림 2-SegmentScopeVsContainerScope](assets/figure2-segment-scope-vs-container-scope.png)

>[!NOTE]
>
>Adobe은 유효하고 잘못된 세그먼트를 이해하는 논리가 있으며, 다음과 같은 옵션을 제공하지 않습니다. *절대 안 함* 작업... 따라서 히트 범위 세그먼트 내에서 방문자 범위 컨테이너를 사용하는 옵션이 표시되면 유효한 옵션입니다.

기본 세그먼트의 경우와 마찬가지로 중첩된 컨테이너가 있는 복잡한 세그먼트를 만들기 시작할 때 다음에 대한 명확한 아이디어가 있어야 합니다. ***내용*** 반환할 데이터의 유형입니다. ***방법*** 해당 데이터를 사용할 계획입니까? ***대상*** 지표를 세그먼트와 연결할 계획입니까?

이러한 질문은 전체 세그먼트의 범위를 결정하는 데 도움이 됩니다. 이것이 모든 세그먼트의 시작점입니다.

세그먼트를 고유 방문자 수 지표와 쌍을 이루려고 한다고 해서 세그먼트 자체가 방문자 수준이 되어야 하는 것은 아닙니다. ... 여기에서 멀리 떨어져 있어야 합니다. 방문자 수준 세그먼트는 방문자에 대한 모든 데이터를 반환합니다. 이는 모든 방문, 모든 페이지 보기 등을 의미합니다. 방문자가 세그먼트 기준과 일치하면 세그먼트는에서 데이터를 반환하기 시작할 수 있습니다. *과거* (작업 영역의 날짜 범위 내에 있는 한) 이 방문자의 경우.

>[!IMPORTANT]
>
>세그먼트를 고유 방문자 수 지표와 페어링하려는 경우에도 *다음을 의미하지 않음* 세그먼트가 자동으로 방문자 범위가 지정되도록 하는 것... 이러한 오해 *힘* 부풀려지고 잘못된 결과를 생성합니다.
...
또 다른 질문은 **이유** 우리 히트곡에 의해 후퇴하고 있습니까? 기술적으로 여기서 방문 횟수 또는 방문자 수를 늘릴 수 있지만, 다음 방법으로 이러한 특정 페이지를 볼 수도 있습니다. **방문당 페이지 보기 수 (특정 페이지 세트용)** 또는 **방문자당 페이지 보기 수(특정 세트용)**, 이 범위를 통해 특정 계산을 유연하게 수행할 수 있습니다. 이러한 히트는 방문 횟수 또는 고유 방문자 수와 쉽게 결합하여 이러한 페이지를 보는 방문 횟수 또는 방문자 수를 결정할 수 있으므로 모든 시나리오에 사용할 수 있는 가장 유연한 세그먼트를 선택합니다.

먼저 비교를 위해 특정 페이지에 대한 간단한 히트 기반 세그먼트를 살펴보겠습니다.

![Segment3A-HitLevelPage](assets/segment-example-3/segment3a-hit-level-pages.png)
...
![Segment3C-ComparisonTable-Detail1](assets/segment-example-3/segment3c-comparison-table-detail1.png) 처음 세 열(페이지 보기 수, 방문 횟수 및 고유 방문자 수)은 세그먼트화되지 않으므로 사이트 내의 모든 페이지를 표시합니다. 주문이 작업에 대해 추적되므로 여기에 주문이 포함되지 않았으므로 페이지 차원의 범위에 포함되지 않습니다.

![Segment3C-ComparisonTable-Detail2](assets/segment-example-3/segment3c-comparison-table-detail2.png) 다음으로 단순세그먼트만 보고 그 결과를 보여주고 있습니다. **히트 수** 지정한 두 페이지에서 분류에 있는 다른 모든 페이지는 예상대로 0이 됩니다.

![Segment3C-ComparisonTable-Detail3](assets/segment-example-3/segment3c-comparison-table-detail3.png) 이제 약간의 추가 팁이 있습니다. 고급 세그먼트의 결과를 표시하기 전에 &quot;주문이 있음&quot;의 다른 간단한 세그먼트를 사용했으며(히트 수준 범위에서) 이를 고유 방문자와 연결했습니다. 이렇게 하면 보고서 기간에 주문한 총 UV와 각 페이지에 도달한 UV가 반환됩니다. 다음 열 세트를 더 잘 설명하는 데 도움이 됩니다.

![Segment3C-ComparisonTable-Detail4](assets/segment-example-3/segment3c-comparison-table-detail4.png) 최종 열 세트는 복잡한 세그먼트로 스택됩니다. 주문이 있는 전체 UV는 각 페이지에서 간단한 &quot;주문이 있음&quot; 세그먼트와 일치하지만, 합계가 상당히 다르다는 것을 알 수 있습니다. 이 데이터 세트는 주문을 하고 페이지를 히트한 방문자로만 데이터 세트를 명시적으로 제한하므로 이에 대해 확실히 관심이 있습니다.
...
이 방문자 범위 세그먼트의 이점은 다음과 같습니다 **모두** 이 기준과 일치하는 방문자의 방문입니다. 따라서 이 조합으로 이어지는 이전 방문의 비헤이비어와 이러한 시나리오 이후 방문자의 작업을 보려는 경우 이 세그먼트가 좋을 것입니다.
...
- 페이지 B
   - **eVar** 및 **eVar** &quot;프로모션 배너&quot;로 설정됩니다.
   - **eVar1의 인스턴스** 트리거됨
   - **eVar2 인스턴스** 트리거됨
- 페이지 C
   - 모두 **eVar** 및 **eVar** &quot;promo-banner&quot; 값 유지
   - 두 eVar 모두 지속된 값을 사용하고 있으므로 eVar에 대한 인스턴스 지표는 모두 실행되지 않습니다

**방문 2**

- 페이지 D
   - **eVar** 이 어떤 값으로도 설정되지 않고 no **eVar1의 인스턴스** 트리거됨
   - **eVar** 30일 만료로 인한 &quot;프로모션 배너&quot; 값 유지
   - **eVar2 인스턴스** 은 값이 지속적이고 실제로 설정되지 않으므로 트리거되지 않습니다.
- URL에서 사이드 레일 프로모션 을 클릭합니다(?icid=promo-side-rail).
- 페이지 E
   - **eVar** 및 **eVar** 이 &quot;프로모션 사이드 레일&quot;로 설정되어 있습니다.
   - **eVar1의 인스턴스** 트리거됨
   - **eVar2 인스턴스** 트리거됨
- 페이지 F
   - 모두 **eVar** 및 **eVar** &quot;프로모션 사이드 레일&quot; 값 유지
   - 두 eVar 모두 지속된 값을 사용하고 있으므로 eVar에 대한 인스턴스 지표는 모두 실행되지 않습니다
...
![그림 4-속성 모델](assets/figure4-attribution-model.png)

*차원의 톱니바퀴 아이콘에서 속성을 설정할 수 있습니다. 각 옵션에는 마우스로 &quot;?&quot;를 가리키면 사용할 수 있는 정보가 있습니다. 아이콘. 기본적으로*
...
우리 모두가 알고 있는 바와 같이, 마케팅 채널에는 긴 속성 모델(기본적으로 30일이지만 사용자 자신의 필요에 따라 맞춤화할 수 있음)이 있으며, 일단 설정되면 특정 드라이버가 전환 속성을 가져오도록 후속 &quot;직접&quot; 사이트 방문으로 마케팅 채널을 덮어쓰지 않습니다. 그러나 경우에 따라 다음을 구체적으로 확인해야 합니다. ***항목*** 마케팅 채널이 마케팅 처리 규칙을 기반으로 특별히 언제 설정되는지 특정 마케팅 채널 및 항목별로 확인해야 합니다.

상황을 좀 더 발전시켜 보고 먼저 비교를 시작한 다음, 세그먼트를 파헤쳐 보겠습니다.

![세그먼트5A-테이블 비교](assets/segment-example-5/segment5a-table-comparison.png)

![Segment5A-TableComparison-Detail1](assets/segment-example-5/segment5a-table-comparison-detail1.png) 처음 4개의 열은 세분화되지 않았으므로 이해하기 쉬워야 합니다. 참고: *&quot;항목&quot;* 는 기본적으로 방문자가 세션을 시작하는 위치를 기반으로 계산된 값입니다. 사용자가 여러 마케팅 채널(소셜 미디어 조회, 검색, 마케팅 이메일 클릭 등을 통해 사이트)을 통해 입력할 수 있으므로 이것이 우리가 찾는 정보를 반환하지 않는다는 것을 보여주기 위해 여기에 추가했습니다. 모두 동일한 방문/세션 내에 있어야 합니다.)

![Segment5A-TableComparison-Detail2](assets/segment-example-5/segment5a-table-comparison-detail2.png) 다음 열 세트는 &quot;표준 히트 세그먼트&quot;를 사용하며, 기본적으로 마케팅 채널이 &quot;유료 검색&quot;인 히트를 봅니다. 하지만 이렇게 하면 마케팅 채널 속성에 따라 모든 히트가 반환되고 실제 &quot;유료 검색&quot; 클릭스루가 분리되지 않습니다. 따라서 필요한 데이터가 반환되지 않습니다.

![Segment5A-PaidSearchHit](assets/segment-example-5/segment5a-paid-search-hit.png)

![Segment5A-TableComparison-Detail3.png](assets/segment-example-5/segment5a-table-comparison-detail3.png) 이제, 다음 두 세트의 데이터는 동일하게 보입니다. 그리고 실제로 이 두 세트는 동일한 데이터를 두 가지 다른 방식으로 반환합니다. 하지만 지금은 구체적으로 *인스턴스* 마케팅 채널 위치 **set** &quot;유료 검색&quot;으로 변경되었습니다.

이 작업은 다음 두 가지 방법으로 수행할 수 있습니다.

먼저, &quot;표준&quot; 차원 속성을 사용하고 이것을 특정 &quot;마케팅 채널 인스턴스&quot; 지표와 연결합니다( *존재함* 논리):
...
![Fire_Buny](assets/fire-bunny.jpeg)
