...
**1단계** - 영업 프로세스에 중요하거나 조직에 중요한 잠재 활동 목록을 만듭니다. 영업 팀과 협력하여 잠재 고객이 구매 의도를 가지고 있음을 나타내는 활동을 결정하는 것이 도움이 될 수 있으며, 이를 통해 판매 실적에 맞게 기준을 조정하고 마감된 거래 관측에 따라 우선 순위를 지정할 수 있습니다. 다음은 영업 팀에 요청할 수 있는 몇 가지 제안 질문입니다.
...
| **구매 의도를 나타내는 활동** | **구매 의사가 없음을 나타내는 활동** |
| --- | --- |
...
**4단계** - &#39;예 1a - 구매 의도를 나타내는 활동&#39;에 나열된 각 활동에 점수를 지정합니다.
...
**5단계** - 점수는 반복적인 과정이라는 것을 기억하십시오! 분석을 위해 더 많은 데이터를 수집할 때 점수와 임계값을 지속적으로 검토하고 조정합니다.
...
**2단계** - 이상적인 잠재 고객 프로필에서의 관련성에 따라 각 특성에 점수를 지정합니다. 리드가 제품에 적합하지 않도록 하는 바람직한 트레이트에는 양수 점수를, 트레이트에는 음수 점수를 사용하십시오.

**예제 2b - 이상적인 및 바람직하지 않은 잠재 고객 특성에 점수 할당**
...
**1단계** - 연습 1과 2의 활동 및 특성을 가져와서 위치나 제품 라인에 따라 다른지 여부를 각 항목에 대해 결정합니다.

**예제 3a - 글로벌 및 로컬 시장의 신호:**
...
| **인구 통계 지표** | **우선 순위 1** | **우선 순위 2** | **우선 순위 3** |
| --- | --- | --- | --- |
| 높은 값 | 20포인트 | 10포인트 | 7점 |
| 중간 값 | 10포인트 | 7점 | 3포인트 |
| 낮은 값 | 5포인트 | 3포인트 | 1포인트 |
...
* 다운로드 [개인 채점 연습 시트](./assets/build-person-scoring-model-and-local-flexibility-in-adobe-marketo-engage.docx){target="_blank} 채점 모델을 오프라인으로 개발하기 위해
* Marketo Engage에서 개인 점수를 매깁니다. 확인 [튜토리얼](https://experienceleague.adobe.com/en/docs/marketo-learn/tutorials/lead-and-data-management/lead-scoring-watch){target="_blank} 및 [데모](https://experienceleague.adobe.com/en/docs/events/marketo-and-mochas-recordings/2023/lead-scoring){target="_blank} 시작합니다. 가망 고객/개인 채점 프로그램을 임포트할 수 있습니다 [템플릿](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/core-marketo-concepts/programs/working-with-programs/import-a-program){target="_blank} Marketo Engage 를 사용하십시오.
...
* 프로그램 빌드가 완료되면 스마트 캠페인을 테스트합니다. 예를 들어에서 데모 양식으로 이동하여 테스트 이메일로 작성한 다음 테스트 개인의 점수를 확인합니다. [Marketo Engage 데이터베이스](https://experienceleague.adobe.com/en/docs/marketo/using/getting-started-with-marketo/quick-wins/simple-scoring#step-view-the-person-info){target="_blank}.
* 모델을 만든 후 개인의 점수가 판매 핸드오프 임계값에 도달하면 판매로 이동하도록 경고를 설정하는 것이 좋습니다. 이를 통해 경고 설정에 대한 자세한 정보 [튜토리얼](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/core-marketo-concepts/smart-campaigns/flow-actions/send-alert){target="_blank}.
